...
public class ReadComponentsFromFile {
    ...
    public static BikeComponent parseLine(String productLine, String splitRegexp) {
        ...
        String[] components = productLine.split(splitRegexp);
        ...
        try {
            ...
            String componentName = components[1];
            String manufacturer = components[2];
            double unitPrice = Double.parseDouble(components[3]);

            return new BikeComponent(componentId, componentName, manufacturer, unitPrice);
...
/**
    ...
    public static ResultSet read(InputStream stream) throws IOException {
        ...
        List<IBikeComponent> components = new ArrayList<>();
        List<Integer> errors = new ArrayList<>();

        ResultSet result = new ResultSet(components, errors);
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(stream))) {
            ...
            while ((line = reader.readLine()) != null) {
                i++;
                BikeComponent parsedComponent = parseLine(line, CSV_SPLIT_REGEX);
