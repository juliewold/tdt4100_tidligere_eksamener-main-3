...
Fill in the enclosed [Temperature](Temperature.java). Temperature has an overview of a temperature that is either in Fahrenheit or in Celisus. 

Temperature has the following methods

- *Temperature(double degrees, char scale)* - Creates a **Temperature** object with a scale (Fahrenheit/Celsisus) and a number of degrees.
- *getScale()* - Returns whether the temperature is in Fahrenheit or Celsius.
- *getDegrees()* - Returns the temperature.
- *toOther()* - Turn this temperature object into the other scale and return it.
