...
public class MaxDistanceRouteFactory implements RouteFactory {
	...
	public Route createRoute(final Post start, final List<Post> posts, final Post finish) {
		final List<Post> routePosts = new ArrayList<>(List.of(start, finish));
		final Iterator<Post> postsIt = posts.iterator();
		while (postsIt.hasNext()) {
			final Post next = postsIt.next();
			routePosts.add(routePosts.size() - 1, next);
			if (Route.distance(routePosts) > maxDistance) {
				routePosts.remove(routePosts.size() - 2);
				break;
			}
		}
		if (routePosts.size() < 3) {
			throw new IllegalArgumentException("Cannot make route with max distance " + maxDistance);
		}
