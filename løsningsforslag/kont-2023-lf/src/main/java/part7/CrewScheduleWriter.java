...
public class CrewScheduleWriter {
    ...
    public void writeCrewScheduleForFlight(ICrewSchedule crewSchedule, IFlight flight, OutputStream outputStream)
        ...
        outputStream.write(flightInfo.getBytes());

        List<CrewMember> assignedCrew = crewSchedule.getAssignedCrewForFlight(flight);
        for (CrewMember crew : assignedCrew) {
            String crewInfo = crew.getName() + " - " + crew.getPosition() + "\n";
            outputStream.write(crewInfo.getBytes());
        }
        outputStream.close();
    }
