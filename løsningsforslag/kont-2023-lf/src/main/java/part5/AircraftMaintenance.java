...
public class AircraftMaintenance {
    ...
    private Map<String, List<IMaintenanceObserver>> observerMap = new HashMap<>();;
    ...
    public void scheduleMaintenance(LocalDateTime startTime, LocalDateTime endTime) {
        ...
        this.updateMaintenanceStatus(MaintenanceStatus.SCHEDULED);
    }
    ...
    public void addObserver(IMaintenanceObserver observer, String fromStatus, String toStatus) {
        ...
        if (!observerMap.containsKey(key)) {
            observerMap.put(key, new ArrayList<>());
        }
        observerMap.get(key).add(observer);
    }
    ...
    public void removeObserver(IMaintenanceObserver observer, String fromStatus, String toStatus) {
        String key = fromStatus + "->" + toStatus;
        if (observerMap.containsKey(key)) {
            observerMap.get(key).remove(observer);
        }
    }
    ...
    public void updateMaintenanceStatus(String maintenanceStatus) {
        if (!MaintenanceStatus.getMaintenanceStatuses().contains(maintenanceStatus)) {
            throw new IllegalArgumentException("Invalid maintenance status");
        }
        String oldStatus = this.maintenanceStatus;
        this.maintenanceStatus = maintenanceStatus;

        // Notify observers
        String key = oldStatus + "->" + maintenanceStatus;
        if (observerMap.containsKey(key)) {
            for (IMaintenanceObserver observer : observerMap.get(key)) {
                observer.maintenanceStatusChanged(aircraft, this.maintenanceStatus);
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(
                "Observer's last seen maintenance status: " + observer.getLastMaintenanceSatus());
