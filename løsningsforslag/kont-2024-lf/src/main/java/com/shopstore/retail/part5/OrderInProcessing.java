...
public class OrderInProcessing implements ProductChangeListener, ProductOrder {
    ...
    private Map<IProduct, Integer> unshippedProducts = new HashMap<>();
    ...
    public OrderInProcessing(ProductOrder productOrder, ProductInventory productInventory) {
        ...
        unshippedProducts.putAll(productOrder.getItems());
    }
    ...
    public boolean isCompleted() {
        return unshippedProducts.isEmpty();
    }
    ...
    public void createShipment() {
        ...
        for (IProduct product : new HashSet<>(unshippedProducts.keySet())) {
            int quantityToShip = unshippedProducts.get(product);
            int currentStock = productInventory.getStock(product);
            if (currentStock < quantityToShip) {
                if (currentStock > 0) {
                    shipment.productsContained().put(product, currentStock);
                }
                unshippedProducts.put(product, quantityToShip - currentStock);
            } else {
                shipment.productsContained().put(product, quantityToShip);
                unshippedProducts.remove(product);
            }
        }
        if (!shipment.productsContained().isEmpty()) {
    ...
    public void createInitialShipment() {
        createShipment();
        if (!unshippedProducts.isEmpty()) {
            productInventory.addListener(this);
        }
    }

    @Override
    public void productChanged(IProduct product, ProductChange productChange) {
        if (unshippedProducts.containsKey(product) && productChange == ProductChange.NOW_AVAILABLE) {
            createShipment();
        }
    }
    ...
    public Customer getCustomer() {
        return productOrder.getCustomer();
    }

    @Override
    public double getTotal() {
        return productOrder.getTotal();
    }

    @Override
    public Map<IProduct, Integer> getItems() {
        return productOrder.getItems();
    }
